...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($slug) || empty($rubro) || empty($dominio) || empty($endpoint_publicar) || empty($api_key_sitio) || empty($email_revisor) || empty($nombre_autor) || empty($foto_autor_url)) {
        ...
        try {
            ...
            $check = $db->prepare("SELECT id FROM clientes WHERE slug = ?");
            $check->execute([$slug]);
            if ($check->fetch()) {
                ...
                $_SESSION['flash_success'] = "Cliente '{$nombre}' registrado con éxito.";
...
// Generar una API key por defecto segura de ejemplo
$default_api_key = bin2hex(random_bytes(16));
...
<div style="margin-bottom: 25px;">
    ...
    <h1 style="font-size: 28px;">Registrar Nuevo Cliente</h1>
</div>
...
<form action="nuevo.php" method="POST">
    <div style="display: grid; grid-template-columns: 1fr; gap: 30px; align-items: start;">
        <?php if (isset($_POST)): ?>
            <!-- Para dispositivos grandes, 2 columnas: Formulario y Live Preview -->
            <div style="display: grid; grid-template-columns: 1fr; gap: 30px;" class="form-grid-wrapper">
        <?php endif; ?>
        ...
        <div class="card-glass">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="nombre" name="nombre" class="form-control" placeholder="Ej: Adri Hair Style" required value="<?php echo htmlspecialchars($_POST['nombre'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label class="form-label" for="slug">Slug Único *</label>
                    <input type="text" id="slug" name="slug" class="form-control" placeholder="Ej: adri-hair-style" required value="<?php echo htmlspecialchars($_POST['slug'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="rubro" name="rubro" class="form-control" placeholder="Ej: peluquería femenina" required value="<?php echo htmlspecialchars($_POST['rubro'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label class="form-label" for="tono_marca">Tono de la Marca *</label>
                    <input type="text" id="tono_marca" name="tono_marca" class="form-control" placeholder="Ej: cercano, moderno, femenino, inspirador" required value="<?php echo htmlspecialchars($_POST['tono_marca'] ?? ''); ?>">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="url" id="dominio" name="dominio" class="form-control" placeholder="Ej: https://adrihairstyle.cl" required value="<?php echo htmlspecialchars($_POST['dominio'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label class="form-label" for="logo_url">URL del Logo (Opcional)</label>
                    <input type="url" id="logo_url" name="logo_url" class="form-control" placeholder="Ej: https://adrihairstyle.cl/assets/logo.png" value="<?php echo htmlspecialchars($_POST['logo_url'] ?? ''); ?>">
                </div>
            </div>

            <div class="form-group" style="margin-top: 15px;">
                <label class="form-label" for="descripcion">Descripción del Negocio (Opcional)</label>
                <textarea id="descripcion" name="descripcion" class="form-control" rows="3" placeholder="Describe detalladamente a qué se dedica el negocio, especialidades y fortalezas..."><?php echo htmlspecialchars($_POST['descripcion'] ?? ''); ?></textarea>
            </div>

            <div class="form-group" style="margin-top: 15px;">
                <label class="form-label" for="temas_relacionar">Temas de Interés a Relacionar (Opcional)</label>
                <textarea id="temas_relacionar" name="temas_relacionar" class="form-control" rows="2" placeholder="Ej: autoestima de la mujer, películas/series empoderadoras, estilo de vida..."><?php echo htmlspecialchars($_POST['temas_relacionar'] ?? ''); ?></textarea>
            </div>
        </div>
        ...
        <div class="card-glass">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="url" id="endpoint_publicar" name="endpoint_publicar" class="form-control" placeholder="Ej: https://adrihairstyle.cl/blog/publicar.php" required value="<?php echo htmlspecialchars($_POST['endpoint_publicar'] ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label class="form-label" for="api_key_sitio">API Key Secreta del Sitio Receptor *</label>
                    <input type="text" id="api_key_sitio" name="api_key_sitio" class="form-control" required value="<?php echo htmlspecialchars($_POST['api_key_sitio'] ?? $default_api_key); ?>">
                </div>
            </div>

            <div class="form-group">
                <label class="form-label" for="email_revisor">Email del Revisor *</label>
                <input type="email" id="email_revisor" name="email_revisor" class="form-control" placeholder="Ej: adri@adrihairstyle.cl" required value="<?php echo htmlspecialchars($_POST['email_revisor'] ?? ''); ?>">
            </div>

            <div class="form-row" style="margin-top: 15px;">
                <div class="form-group">
                    <label class="form-label" for="frecuencia_dias">Frecuencia de Sugerencias (Días) *</label>
                    <input type="number" id="frecuencia_dias" name="frecuencia_dias" class="form-control" min="1" max="365" required value="<?php echo htmlspecialchars($_POST['frecuencia_dias'] ?? '7'); ?>">
                </div>
                <div class="form-group">
                    <label class="form-label" for="limite_mensual_usd">Límite de Gasto Mensual (USD) *</label>
                    <input type="number" step="0.0001" id="limite_mensual_usd" name="limite_mensual_usd" class="form-control" min="0" required value="<?php echo htmlspecialchars($_POST['limite_mensual_usd'] ?? '5.0000'); ?>">
                </div>
            </div>
        </div>

        <!-- Columna de Autor y Estilo (Preview en vivo) -->
        <div style="display: grid; grid-template-columns: 1fr; gap: 30px;" class="responsive-subgrid">
            <div class="card-glass">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nombre_autor" name="nombre_autor" class="form-control" placeholder="Ej: Adri Montenegro" required value="<?php echo htmlspecialchars($_POST['nombre_autor'] ?? ''); ?>">
                </div>

                <div style="display: flex; gap: 20px; align-items: center;">
                    <div style="flex-grow: 1;">
                        <label class="form-label" for="foto_autor_url">URL Foto del Autor (Absoluta) *</label>
                        <input type="url" id="foto_autor_url" name="foto_autor_url" class="form-control" placeholder="https://ejemplo.com/foto.jpg" required value="<?php echo htmlspecialchars($_POST['foto_autor_url'] ?? ''); ?>">
                    </div>
                    
                    <!-- Previsualización en Vivo de la Foto de Autor -->
                    <div style="text-align: center; flex-shrink: 0;">
                        ...
                        <div style="width: 70px; height: 70px; border-radius: 50%; overflow: hidden; border: 2px solid var(--color-primary); background: #1E293B; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
                            <img id="preview-foto-autor" src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=128&h=128&q=80" alt="Avatar" style="width: 100%; height: 100%; object-fit: cover;">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-glass">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <input type="color" id="color_primario_picker" class="form-control" style="width: 50px; padding: 2px; height: 40px; cursor: pointer;" value="<?php echo htmlspecialchars($_POST['color_primario'] ?? '#E8B4B8'); ?>">
                            <input type="text" id="color_primario" name="color_primario" class="form-control" placeholder="#E8B4B8" maxlength="7" required value="<?php echo htmlspecialchars($_POST['color_primario'] ?? '#E8B4B8'); ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="color_texto">Color de Texto *</label>
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <input type="color" id="color_texto_picker" class="form-control" style="width: 50px; padding: 2px; height: 40px; cursor: pointer;" value="<?php echo htmlspecialchars($_POST['color_texto'] ?? '#2C2C2A'); ?>">
                            <input type="text" id="color_texto" name="color_texto" class="form-control" placeholder="#2C2C2A" maxlength="7" required value="<?php echo htmlspecialchars($_POST['color_texto'] ?? '#2C2C2A'); ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="fuente_titulo" name="fuente_titulo" class="form-control" required>
                            <option value="Georgia, serif" <?php echo (($_POST['fuente_titulo'] ?? '') === 'Georgia, serif') ? 'selected' : ''; ?>>Georgia (Serif Elegante)</option>
                            <option value="'Outfit', sans-serif" <?php echo (($_POST['fuente_titulo'] ?? '') === "'Outfit', sans-serif") ? 'selected' : ''; ?>>Outfit (Moderna Geométrica)</option>
                            <option value="'Playfair Display', serif" <?php echo (($_POST['fuente_titulo'] ?? '') === "'Playfair Display', serif") ? 'selected' : ''; ?>>Playfair Display (Premium Editorial)</option>
                            <option value="system-ui, sans-serif" <?php echo (($_POST['fuente_titulo'] ?? '') === 'system-ui, sans-serif') ? 'selected' : ''; ?>>System UI (Limpia por Defecto)</option>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="fuente_texto" name="fuente_texto" class="form-control" required>
                            <option value="system-ui, sans-serif" <?php echo (($_POST['fuente_texto'] ?? '') === 'system-ui, sans-serif') ? 'selected' : ''; ?>>System UI (Limpia por Defecto)</option>
                            <option value="'Inter', sans-serif" <?php echo (($_POST['fuente_texto'] ?? '') === "'Inter', sans-serif") ? 'selected' : ''; ?>>Inter (Altamente Legible)</option>
                            <option value="Georgia, serif" <?php echo (($_POST['fuente_texto'] ?? '') === 'Georgia, serif') ? 'selected' : ''; ?>>Georgia (Estilo Literario)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group" style="margin-top: 10px;">
                    <label style="display: inline-flex; align-items: center; gap: 10px; cursor: pointer;">
                        <input type="checkbox" name="activo" value="1" checked style="width: 18px; height: 18px; accent-color: var(--color-primary);">
        ...
        <div style="display: flex; gap: 15px; justify-content: flex-end; margin-top: 10px;">
            ...
            <button type="submit" class="btn-custom btn-primary">Guardar Cliente</button>
        </div>
    </div>
</form>
